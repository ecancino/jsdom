...
exports.createStylesheet = (sheetText, elementImpl, baseURL) => {
  let sheet;
  try {
    const ast = cssTools.parse(sheetText, { silent: true });

    ast.stylesheet.rules = ast.stylesheet.rules.flatMap(rule => {
      return rule.type === "layer" ? rule.rules : rule;
    });

    sheet = cssom.parse(cssTools.stringify(ast));
  } catch (e) {
